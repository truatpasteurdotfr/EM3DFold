# This file is adapted from ModelAngelo
# In the orignal file, NAs are reprented with local frame defined by OP1-P-OP2 atoms
# In this file, NAs are represented with local frame defined by C3'-C4'-O4' atoms
# The atom positions in the rigid groups, the torsion angles difinition are also changed
# The Protein class is converted to Polymer class (a.k.a protein and NA)
...
from em3dfold.polymer_utils import residue_constants as _rc
...
class Polymer:
    ...
    # 0-indexed number corresponding to the chain that this residue
    # belongs to.
    ...
    # 11 Frames corresponding to 'atom_positions'
    # represented as affines.
    rigidgroups_gt_frames: np.ndarray  # (num_res, 11, 3, 4)
    ...
    rigidgroups_gt_exists: np.ndarray  # (num_res, 11)

    # Mask denoting whether given group is in
    # principle present for given amino acid type.
    rigidgroups_group_exists: np.ndarray  # (num_res, 11)

    # Mask denoting whether frame is
    # affected by naming ambiguity.
    rigidgroups_group_is_ambiguous: np.ndarray  # (num_res, 11)

    # 11 Frames with alternative atom renaming
    # corresponding to 'all_atom_positions' represented as affines
    rigidgroups_alt_gt_frames: np.ndarray  # (num_res, 11, 3, 4)
    ...
    def to_float32(self) -> 'Polymer':
        field_dict = dataclasses.asdict(self)
        converted_dict = {}
        for field_name, value in field_dict.items():
            if isinstance(value, np.ndarray) and np.issubdtype(value.dtype, np.floating):
                converted_dict[field_name] = value.astype(np.float32)
            else:
                converted_dict[field_name] = value
        return Polymer(**converted_dict)

    def to_float32_inplace(self) -> None:
        for field in dataclasses.fields(self):
            value = getattr(self, field.name)
            if isinstance(value, np.ndarray) and np.issubdtype(value.dtype, np.floating):
                setattr(self, field.name, value.astype(np.float32))


def get_polymer_from_file_path(file_path: str, chain_id: str = None) -> Polymer:
    """Takes a file path containing a PDB/mmCIF file and constructs a Polymer object.
    ...
    Returns:
      A new `Polymer` parsed from the pdb contents.
    """
...
def get_polymer_empty_except(**kwargs) -> Polymer:
    polymer_dict = dict.fromkeys(POLYMER_KEYS)
    for (k, v) in kwargs.items():
        polymer_dict[k] = v
    return Polymer(**polymer_dict)


def get_alt_atomc_positions(
    aatype,
    atomc_positions,
    atomc_mask=None,
):
    """Apply ambiguous atom-label swaps to atom23/atomc coordinates."""
    is_torch = torch.is_tensor(atomc_positions)
    alt_atomc_positions = atomc_positions.clone() if is_torch else atomc_positions.copy()
    alt_atomc_mask = None
    if atomc_mask is not None:
        alt_atomc_mask = atomc_mask.clone() if torch.is_tensor(atomc_mask) else atomc_mask.copy()

    if torch.is_tensor(aatype):
        aatype_np = aatype.detach().cpu().numpy()
    else:
        aatype_np = np.asarray(aatype)

    for resname, swaps in _rc.residue_atom_position_swaps.items():
        restype = _rc.restype_order[_rc.restype_3to1[resname]]
        residue_indices = np.where(aatype_np == restype)[0]
        if len(residue_indices) == 0:
            continue

        atom_names = _rc.restype_name_to_atomc_names[resname]
        if is_torch:
            residue_indices_t = torch.as_tensor(
                residue_indices,
                device=alt_atomc_positions.device,
                dtype=torch.long,
            )

        for atom_name_a, atom_name_b in swaps.items():
            atom_idx_a = atom_names.index(atom_name_a)
            atom_idx_b = atom_names.index(atom_name_b)

            if is_torch:
                tmp = alt_atomc_positions[residue_indices_t, atom_idx_a].clone()
                alt_atomc_positions[residue_indices_t, atom_idx_a] = alt_atomc_positions[
                    residue_indices_t, atom_idx_b
                ]
                alt_atomc_positions[residue_indices_t, atom_idx_b] = tmp

                if alt_atomc_mask is not None:
                    tmp_mask = alt_atomc_mask[residue_indices_t, atom_idx_a].clone()
                    alt_atomc_mask[residue_indices_t, atom_idx_a] = alt_atomc_mask[
                        residue_indices_t, atom_idx_b
                    ]
                    alt_atomc_mask[residue_indices_t, atom_idx_b] = tmp_mask
            else:
                tmp = alt_atomc_positions[residue_indices, atom_idx_a].copy()
                alt_atomc_positions[residue_indices, atom_idx_a] = alt_atomc_positions[
                    residue_indices, atom_idx_b
                ]
                alt_atomc_positions[residue_indices, atom_idx_b] = tmp

                if alt_atomc_mask is not None:
                    tmp_mask = alt_atomc_mask[residue_indices, atom_idx_a].copy()
                    alt_atomc_mask[residue_indices, atom_idx_a] = alt_atomc_mask[
                        residue_indices, atom_idx_b
                    ]
                    alt_atomc_mask[residue_indices, atom_idx_b] = tmp_mask

    if atomc_mask is None:
        return alt_atomc_positions
    return alt_atomc_positions, alt_atomc_mask
...
def atomf_to_frames(
    ...
    for restype, restype_letter in enumerate(_rc.index_to_restype_1):
        ...
        if _rc.restype3_is_prot(resname):
            ...
            # NAs
            # 0: backbone frame
            restype_rigidgroup_base_atom_names[restype, 0, :] = ["O4'", "C4'", "C3'"]

            # 1-10: others
            for chi_idx in range(10):
                if _rc.chi_angles_mask[restype][chi_idx]:
                    atom_names = _rc.chi_angles_atoms[resname][chi_idx]
                    restype_rigidgroup_base_atom_names[
                        restype, chi_idx + 1, :
    ...
    # Proteins
    chi_angles_mask = np.asarray(_rc.chi_angles_mask)
    restype_rigidgroup_mask[: _rc.num_prot, 0] = 1
    restype_rigidgroup_mask[: _rc.num_prot, 3] = 1
    n = restype_rigidgroup_mask.shape[1] - 4
    restype_rigidgroup_mask[: _rc.num_prot, 4:] = chi_angles_mask[: _rc.num_prot, :n]

    # NAs
    restype_rigidgroup_mask[_rc.num_prot :, :] = 1
    restype_rigidgroup_mask[
        _rc.num_prot : _rc.num_prot + 4, 8
    ] = 0  # DNA does not have frame of idx-8 (no O2' atom)
    ...
    gt_exists = np.min(gt_atoms_exist, axis=-1) * group_exists  # (N, 11)
...
def atomf_to_torsion_angles(
    ...
    """Computes the torsion angles (in sin, cos encoding) for each residue.
    For proteins, the 7 torsion angles are in the order
    '[pre_omega, phi, psi, chi_1, chi_2, chi_3, chi_4]'.
    For NAs, there are 10 torsion angles.
    ...
    Returns:
      Dict containing:
        * 'torsion_angles_sin_cos': Array with shape (B, N, 10/11, 2) where the final
          2 dimensions denote sin and cos respectively
    ...
    torsion_angles_mask[prot_mask, 2] = (
        ...
        * all_atom_mask[prot_mask, 3]  # this O
    )  # this O
    ...
    # Select atoms to compute angles. Shape: [batch, num_aa_res, angles=10, atoms=4].
    ...
    # Gather atom positions. Shape: [len(na_aatype), angles=10, atoms=4, xyz=3].
    torsions_atom_pos[~prot_mask, :10] = np.take(
    ...
    torsion_angles_mask[~prot_mask, :10] = na_mask * (na_atoms_mask).astype(np.float32)

    # Collect the atoms for the chi-angles.
    # Compute the table of chi angle indices. Shape: [restypes, chis=10, atoms=4].
    chi_atom_indices = _rc.chi_atom_indices
    # Select atoms to compute chis. Shape: [batch, num_res, chis=10, atoms=4].
    ...
    # np.ndarray (B, N, torsions=10, sincos=2)
    ...
    mirror_torsion_angles_prot = np.concatenate(
        [
            np.ones([num_batch, num_res, 3]), # bb 3
            1.0 - 2.0 * chi_is_ambiguous[..., :4], # chi 1-4
            np.ones([num_batch, num_res, 1]), # extra chi 5
            np.zeros([num_batch, num_res, 2]), # dummy
        ],
        axis=-1,
    )
    mirror_torsion_angles_na = 1.0 - 2.0 * chi_is_ambiguous

    # Get alts
    alt_torsion_angles_sin_cos = torsion_angles_sin_cos.copy()
    alt_torsion_angles_sin_cos[prot_mask] *= mirror_torsion_angles_prot[prot_mask][..., None]
    alt_torsion_angles_sin_cos[~prot_mask] *= mirror_torsion_angles_na[~prot_mask][..., None]
    ...
    return {
        "torsion_angles_sin_cos": torsion_angles_sin_cos,  # (B, N, 10/11, 2)
        "alt_torsion_angles_sin_cos": alt_torsion_angles_sin_cos,  # (B, N, 10/11, 2)
        "torsion_angles_mask": torsion_angles_mask,  # (B, N, 10/11)
    }
...
def torsion_angles_to_frames(
    ...
    torsion_angles_sin_cos: torch.Tensor,  # (N, 10, 2)
):  # (N, 11)
    ...
    assert torsion_angles_sin_cos.shape[1] == 10
    ...
    # Affines with shape (N, 11, 3, 4)
    m = _rc.restype_rigid_group_default_frame[aatype]

    default_frames = affine_from_tensor4x4(torch.Tensor(m).to(device)).to(dtype)
    ...
    sin_angles = torch.cat(
        [torch.zeros(num_residues, 1, device=device, dtype=dtype), sin_angles], dim=-1
    )
    cos_angles = torch.cat(
        [torch.ones(num_residues, 1, device=device, dtype=dtype), cos_angles], dim=-1
    )
    zeros = torch.zeros_like(sin_angles)
    ones = torch.ones_like(sin_angles)

    # all_rots are rotation_matrices with shape (N, 10, 3, 3)
    ...
    all_frames = affine_mul_rots(default_frames, all_rots).to(dtype)
    ...
    # Nucleotides:
    backbone2_atom1_frame = all_frames[~prot_mask, 1]   # C2'
    backbone2_atom2_frame = all_frames[~prot_mask, 2]   # C1'
    backbone2_atom3_frame = all_frames[~prot_mask, 3]   # N9/N1
    delta_frame_to_frame  = all_frames[~prot_mask, 4]   # O3'
    gamma_frame_to_frame  = all_frames[~prot_mask, 5]   # O5'
    beta_frame_to_frame   = all_frames[~prot_mask, 6]   # P
    alpha1_frame_to_frame = all_frames[~prot_mask, 7]   # OP1
    alpha2_frame_to_frame = all_frames[~prot_mask, 8]   # OP2
    tm_frame_to_frame     = all_frames[~prot_mask, 9]   # O2'
    chi_frame_to_frame    = all_frames[~prot_mask, 10]  # base atoms
    ...
    backbone2_atom3_frame_to_bb = affine_composition(
        backbone2_atom2_frame,
        backbone2_atom3_frame,
    )
    delta_frame_to_bb = delta_frame_to_frame
    gamma_frame_to_bb = gamma_frame_to_frame
    beta_frame_to_bb = affine_composition(
        gamma_frame_to_bb,
        beta_frame_to_frame,
    )
    alpha1_frame_to_bb = affine_composition(
        beta_frame_to_bb,
        alpha1_frame_to_frame,
    )
    alpha2_frame_to_bb = affine_composition(
        beta_frame_to_bb,
        alpha2_frame_to_frame,
    )
    ...
    tm_frame_to_bb = affine_composition(
        backbone2_atom1_frame_to_bb,
        tm_frame_to_frame,
    )
    chi_frame_to_bb = affine_composition(
        backbone2_atom3_frame_to_bb,
        chi_frame_to_frame,
    )

    all_frames_to_backb[~prot_mask] = torch.stack(
        [
            all_frames[~prot_mask, 0],
            backbone2_atom1_frame_to_bb,
            backbone2_atom2_frame_to_bb,
            backbone2_atom3_frame_to_bb,
            delta_frame_to_bb,
            gamma_frame_to_bb,
            beta_frame_to_bb,
...
def frames_and_literature_positions_to_atomc_pos(
    aatype: np.ndarray,
    all_frames_to_global: torch.Tensor,  # (N)  # (N, 11, 3, 4)
    return_mask=False,
):  # (N, 23, 3)
    ...
    group_mask = torch.eye(11, device=device)[residx_to_group_idx.reshape(-1)].reshape(
        *residx_to_group_idx.shape, 11
    )  # shape (N, 23, 11)
...
def add_lm_embeddings_to_polymer(
    input_polymer: Polymer, lm_embeddings: np.ndarray
) -> Polymer:
    # assert len(lm_embeddings) == input_polymer.unified_seq_len
    polymer_dict_without_lm = dict(
        [
            (k, v)
            for (k, v) in input_polymer.__dict__.items()
            if "residue_to_lm_embedding" not in k
        ]
    )
    new_polymer = Polymer(
        **polymer_dict_without_lm, residue_to_lm_embedding=lm_embeddings,
    )
    return new_polymer


def load_polymer_from_pkl(file_path):
    with open(file_path, "rb") as f:
        data = pickle.load(f)
    return Polymer(**data)


def dump_polymer_to_pkl(input_polymer, file_path):
    with open(file_path, "wb") as f:
        pickle.dump(input_polymer.__dict__, f)
...
def make_up_fr_from_polymer(poly: Polymer):
    final_results = {}
    final_results["existence_mask"] = np.ones(len(poly.aatype)) > 0.5
    final_results["aa_logits"] = np.zeros((len(poly.aatype, 28)))
    final_results["aa_logits"][np.arange(len(poly.aatype)), poly.aatype] = 2
    final_results["pred_affines"] = poly.rigidgroups_gt_frames[:, 0]


def slice_polymer(polymer: Polymer, slice_array: np.ndarray) -> Polymer:
    num_res = len(polymer.aatype)
    for key in POLYMER_KEYS:
        value = getattr(polymer, key)
        if hasattr(value, "shape") and value.shape[0] == num_res:
            setattr(polymer, key, value[slice_array])
    return polymer
...
if __name__ == '__main__':
    import sys
    import torch
    from em3dfold.io.pdbio import read_pdb, chains_atom_pos_to_pdb, split_to_chains

    polymer = get_polymer_from_file_path(
        sys.argv[1],
    )

    prot_mask = polymer.prot_mask
    torsion_angles_sin_cos = polymer.torsion_angles_sin_cos
    alt_torsion_angles_sin_cos = polymer.alt_torsion_angles_sin_cos

    frames = torsion_angles_to_frames(
        polymer.aatype,
        torch.from_numpy(polymer.rigidgroups_gt_frames[:, 0]),
        torch.from_numpy(torsion_angles_sin_cos),
        #torch.from_numpy(alt_torsion_angles_sin_cos),
    )

    print("gt", polymer.rigidgroups_gt_frames.shape)
    print("convert", frames.shape)

    pos = frames_and_literature_positions_to_atomc_pos(
        polymer.aatype,
        #torch.from_numpy(polymer.rigidgroups_gt_frames[..., :3, :4]),
        frames,
    ).float().numpy()
    pos = pos * polymer.atomc_mask[..., None]

    print(polymer.atomc_positions[0])
    exit()
