...
void print_help(const char* arg) {
	cout << endl
		<< " Usage: " << arg << " PDB.pdb" << "  chain" << " stride_ss " << endl << endl;
...
int main(int argc, const char* argv[])
{
    ...
    int n_min_res = 50;
    //if (argc > 4){
    //   n_min_res = std::stoi(argv[4]);
    //}

    bool secstr = false;
    for(int i = 0; i < argc; ++i){
        if ( std::string(argv[i]) == "--secstr" ){
            secstr = true;
        }
    }
...
//cout << "chain="<< chain << "!" <<endl;	
	...
	if (tag == 0) {
        if (secstr == false) {
		    ...
		    if (string::size_type id = Cut_Domain_sec_x(ContactMatrix, W, N, vsec).find(";") != string::npos) {
		    	//find the subdomain
		    	vector<string> domain;
		    	Split_String(Cut_Domain_sec_x(ContactMatrix, W, N, vsec), domain, ";");
		    	for (int j = 0; j < domain.size(); j++) {
		    		initialFrag.push_back(domain[j]);
		    	}
		    }
		    else {
		    	initialFrag.push_back(Cut_Domain_sec_x(ContactMatrix, W, N, vsec));
		    }
		    do {
		    	t = 0;
		    	vector<string> finalFrag;
		    	for (unsigned int j = 0; j < initialFrag.size(); j++)
		    	{
		    		...
		    		N1 = string_to_vector(initialFrag[j], N, N);
		    		V1 = string_to_vector(initialFrag[j], vsec, N);
		    		if (string::size_type id = Cut_Domain_sec_x(ContactMatrix, W1, N1, V1).find(";") != string::npos) {
		    			Split_String(Cut_Domain_sec_x(ContactMatrix, W1, N1, V1), spiltFrag, ";");
		    			for (unsigned int i = 0; i < spiltFrag.size(); i++) {
		    				finalFrag.push_back(spiltFrag[i]);
		    			}
		    			t += 1;
		    			...
		    			finalFrag.push_back(Cut_Domain_sec_x(ContactMatrix, W1, N1, V1));
		    		}
		    	}
		    	initialFrag.swap(finalFrag);
		    } while (t > 0);
	...
	do {
		...
		for (unsigned int i = 0; i < initialFrag.size(); i++) {
            ...
            //MOD
            int l0 = W1.size();
            bool i_too_short = l0 < n_min_res;
            //cout << i_too_short << endl;

            int best_j_to_be_merged = -1;
            double best_j_score = -100000;
			...
			for (int j = 0; j < i; j++) {
                ...
                //MOD by TL
                if (i_too_short){
                    if(nnc_W1_W2 > best_j_score){
                        best_j_score = nnc_W1_W2;
                        best_j_to_be_merged = j;
                    }
                }
			}

            //cout << best_j_score << ' ' << best_j_to_be_merged << endl;
            //if (best_j_to_be_merged != -1 && merged.count(i) == 0 && merged.count(best_j_to_be_merged) == 0){
            //    combineFrag = initialFrag[i] + ";" + initialFrag[best_j_to_be_merged];
            //    merged.insert(i);
            //    merged.insert(best_j_to_be_merged);
            //}
            //cout << combineFrag << endl;
		}
