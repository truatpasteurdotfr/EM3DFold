...
from em3dfold.utils.torch_utils import is_ndarray, shared_cat
...
def init_random_affine_from_translation_np(translation):
    translation = np.asarray(translation, dtype=np.float32)
    v = np.random.rand(*translation.shape).astype(np.float32)
    w = np.random.rand(*translation.shape).astype(np.float32)

    v = v / np.linalg.norm(v, axis=-1, keepdims=True).clip(min=1e-8)
    proj = np.sum(w * v, axis=-1, keepdims=True)
    w = w - proj * v
    w = w / np.linalg.norm(w, axis=-1, keepdims=True).clip(min=1e-8)
    u = np.cross(v, w, axis=-1)

    rot = np.stack((v, w, u), axis=-1).astype(np.float32)
    return get_affine(rot, translation)
